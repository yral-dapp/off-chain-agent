...
use yral_spacetime_bindings::autogenerated::dedup_index::add;
...
pub async fn test_dedup_index_access(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let res = state.dedup_index_ctx.reducers.add(
        "test_hash".into(),
        "test_video_id".into(),
        SystemTime::now().into(),
    );
