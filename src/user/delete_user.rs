...
//         std::env::var("YRAL_AUTH_V2_SIGNING_PUBLIC_KEY")
//             .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, "Missing auth config".to_string()))?,
//     )
//     .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Auth init error: {}", e)))?;
...
//         .await
//         .map_err(|e| (StatusCode::NOT_FOUND, format!("User canister not found: {}", e)))?;
...
//         log::info!("TODO: Implement ML feed cache deletion for user: {}", user_principal);
...
//         .delete(&metadata_url)
//         .header("Authorization", format!("Bearer {}", std::env::var("YRAL_METADATA_TOKEN").unwrap_or_default()))
//         .send()
//         .await
//         .map_err(|e| {
//             (
...
// async fn get_user_posts(bq_client: &Client, canister_id: &str) -> Result<Vec<UserPost>, anyhow::Error> {
//     let query = format!(
//         r#"
//         SELECT
