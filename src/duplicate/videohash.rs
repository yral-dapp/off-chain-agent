...
impl VideoHash {
    ...
    pub fn fast_hash(video_path: &Path) -> Result<String, Box<dyn Error + Send + Sync>> {
        ...
        let output_pattern = temp_dir.join("frame_%04d.jpg").to_string_lossy().to_string();
        ...
        println!("Extracting {} frames took {:?}", selected_frames.len(), start.elapsed());
        let hash_start = Instant::now();
        
        // 6. Load frames in parallel with error handling
        let frames: Vec<_> = selected_frames.par_iter()
    ...
    pub fn calculate_wavelet_hash(frames: &[DynamicImage]) -> Result<Vec<bool>, Box<dyn Error + Send + Sync>> {
        let num_frames = frames.len();
        
        // Fast path for single frame
        if num_frames == 1 {
            let gray = frames[0].resize_exact(GRID_SIZE, GRID_SIZE, FilterType::Triangle).grayscale().to_luma8();
        ...
        let resized_frames: Vec<_> = frames.par_iter()
            .map(|frame| {
                frame.resize_exact(FRAME_SIZE, FRAME_SIZE, FilterType::Triangle).to_rgba8()
            })
            .collect();
    ...
    pub fn calculate_color_hash(frames: &[DynamicImage]) -> Result<Vec<bool>, Box<dyn Error + Send + Sync>> {
        // Fast path for single frame
        if frames.len() == 1 {
        ...
        let total_width = frames.par_iter()
            .map(|frame| {
        ...
        for _y in 0..8 {
            for x in 0..8 {
                ...
                let sample_rate = if chunk_width * chunk_height > 10000 { 4 } else { 1 };
                
                for y_pos in (0..chunk_height).step_by(sample_rate) {
                    for x_pos in (x_start..std::cmp::min(x_start + chunk_width, stitch.width())).step_by(sample_rate) {
    ...
    /// # Returns
    /// * `Result<Vec<bool>, Box<dyn Error + Send + Sync>>` - Color hash as boolean vector
    pub fn calculate_single_frame_color_hash(frame: &DynamicImage) -> Result<Vec<bool>, Box<dyn Error + Send + Sync>> {
        // Resize to 8x8
        let small = frame.resize_exact(GRID_SIZE, GRID_SIZE, FilterType::Triangle).to_rgba8();
        let mut hash_bits = Vec::with_capacity(64);
